...
guessing. The same rule applies: it's read to decide what to show you, never logged or tracked.
...
### What we store on our servers: your base location
...
This is the one piece of location data you enter that Cycling Commons stores server-side, and it is
...
- **What is derived from it is stored too.** Alongside the rounded point, the radius and the place
  name you typed, the server works out which regions and countries your circle covers and keeps
  those ids, so your map view can be scoped without recomputing them every time. They are removed
...
Two other things on our servers touch on location without being a location: the country you pick
for your public profile, and the salted one-way hashes of IP addresses that count uses of the
signed-out tools against a daily limit. Both are described in the
[privacy policy](https://cyclingcommons.org/privacy), which also lists the base location under what
an account holds. That is the authoritative wording; the detail here is about how the *map* uses it.
...
language or your last zoom level. It doesn't track you across sites, it isn't shared with any
...
Analytics sets no cookies, so it doesn't change the banner answer above. What it does collect (it
does receive your IP address, which is personal data) is set out in the
...
securely hashed password, and standard server logs, which record IP addresses), the same as any site
with a login. None of that is used by, or shared with, the location feature described here.
