...
flowchart TD
    ...
    MOD -->|accepted| U["Unverified<br/>public but unconfirmed<br/>wears the ? badge<br/>shows in Everything only"]

    U --> GATE{"Verification gate<br/>X independent riders tap<br/>still here / still true"}
    GATE --> V["Verified<br/>the ? badge is gone"]
...
### The evidence ladder, as the map draws it

Two questions, two marks, and they never share a channel. **Who keeps the
record** is the border: a small disc for a gross provider such as
OpenStreetMap, a dashed ring for a specialty provider with a registered scope
for that kind of place in that region, solid paper for a record this community
keeps. **Has anybody stood there** is the badge: a `?` means nobody has, on
record. That is the whole vocabulary, three borders by two badge states:

| | nobody has stood here | a dated witness is on record |
|---|---|---|
| a gross provider, small disc | ![gross provider, unwitnessed](assets/markers/grid-gross-badge.png) | ![gross provider, witnessed](assets/markers/grid-gross-plain.png) |
...
*live claim* comes from a source that republished the record inside the
freshness window without retracting it. A *witness* is a person at the point on
a known date. The markers below are the map's own, rendered by the curator
page at `/curator/markers` and cropped, never redrawn, so this table cannot
...
saying a viewpoint is there, and none of them saying it is good. Best of is
votes, a layer on top of the ladder, and only for the experiential types
(section 2 above).
