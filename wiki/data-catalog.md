...
**non-personal** (about the *world*, never a person's identity). Never scraped; contributed by riders as reports, edits, and aggregate, anonymous signals.
...
- **Non-personal only.** The dataset carries no identity and nothing about a person: the Commons is the map, not the rider. That is a statement about *this catalogue*, not about the whole service. An account holds ordinary account data, and analytics and server logs see an IP address; the [privacy policy](https://cyclingcommons.org/privacy) is the authoritative account of both.
...
- Traffic exposure: derived from road class, speed limit, lanes and cycleway, and quietness as Scout detects it from rides; the input for a route's sensory load [auto][OSM] *(being built)*. Riders are asked too, and today as one voice: the road form has a traffic field (quiet · moderate · busy · car-free) and the route form a quietness rating. What is not built is the tally, ten say quiet and two say moderate, with the time of day attached: a road is quiet at seven and not at five [edit]
...
- **Seed from open data.** OSM's public GPS traces are already openly contributed, so the heatmap can
...
- Most of A–G, N and O, and parts of P, have **OSM tags already**, so design the Commons to *interoperate with* OSM (import + contribute back under ODbL), so you inherit a huge head start and avoid duplicating the base map.
...
The catalog above is the categories. This is the depth under each one, in
three lines per letter, checked against the improve forms
(`CatalogFormRegistry`, 2026-09-09). **Form** is what the form asks today, in
its own words; **OSM** is what the base map already carries; **Design** is
named and not built. Most forms also ask *still as mapped?* (as mapped · out of
order · closed · not there anymore); that is the freshness question and is not
repeated below.
...
construction. That is one of six buckets the project keeps apart, and it is the only one that can
travel to OSM at all:

| What | Licence |
|------|---------|
| **Data**: this catalog, places, climbs, routes, attributes | ODbL 1.0 |
...
Media travels under a creative-works licence, and photographs do not belong in OSM anyway. The code
licence governs the platform, not the catalog, and has no bearing on either direction the data moves.
The flow, lightest to heaviest:
...
- **Formal import (later, heavy).** Bulk contribution needs an OSM *import plan*, community discussion, an
  ODbL-compatible source, and a dedicated account. The rules are strict, and bad imports get reverted. Not a day-one move.
