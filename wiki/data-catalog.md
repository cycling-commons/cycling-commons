...
- **Media is a first-class, consent-gated contribution.** Photos and video join the Commons under **CC BY-SA 4.0** with provenance — the contributor explicitly confirms they own it and *donate* it (first-time consent), or links a photo from a recognised source whose licence is validated. Never scraped. Always a real capture: nothing generated or composed by AI or any other artificial means, and nothing added to or removed from the scene, with one exclusion: faces and number plates are blurred before upload. Light editing is fine, with AI tools or without: colour toning, cropping, sharpening, noise removal. Marked **[media]**. (Multiple photos per item, plus short video; galleries show as a slideshow.)
