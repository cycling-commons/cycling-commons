...
| **Antimeridian / dateline wrapping** | A region's bounding box spans the entire globe instead of a thin sliver near ±180° | Longitude wraps from +180 back to −180 at the antimeridian. A naive `min`/`max` union of longitudes breaks the instant a shape's coordinates straddle that seam, the two extreme values end up on opposite sides of the world instead of close together | Nothing in this repository handles it, and two onboarded regions need it: the `new-zealand` and `united-states` rows in `region` (the Chatham Islands and the Aleutians cross 180°) span from about −179° to +179° as a naive `min`/`max`, a box the width of the world. `RegionRegistryProvider::all()` (`web/src/Catalog/RegionRegistryProvider.php`) publishes exactly those `ST_XMin`/`ST_XMax` extents, and `CCScope.bbox()` (`web/assets/map/scope.js`) unions them the same naive way, so a viewport fit or a Photon bbox built from either region gets the whole globe. This is a known open defect, not an oversight: `docs/specs/Dated/2026-07-19-region-scoping-design.md` §8 risk 11 names the two paths and the fix (split boxes, or a longitude-normalised union). Until that fix lands, treat any bbox derived from those two regions as unusable, and add the fix before relying on region extents for anything new |
...
confirmed-points pool. The coverage layer never clusters, because a cluster renders at its members'
centroid, a position no scope filter can vouch for (a phantom bubble outside the region); its
overview is a density heatmap instead. [`tiles.md`](tiles.md), [`on-screen.md`](on-screen.md)
...
**Point**: a single coordinate pair and nothing else; nearly every catalog item is stored as one, the
road-surface layer being the exception that stores LineStrings. [`shapes.md`](shapes.md)

**Polygon**: a closed ring of points, optionally with holes cut out, describing an area; a region's
outline is one, stored as a MultiPolygon. [`shapes.md`](shapes.md)
...
than implementing it, and offers no rider-facing A-to-B planner. [`routes.md`](routes.md)
...
!!! tip "Hands-on: diagnose a real zero-row query"
    ...
    how much coverage your stack has loaded, five on the offline fixture, a few hundred once a real
    Belgian extract is in, and that is fine, because the number is not the lesson. The transition from zero
