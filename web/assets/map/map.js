...
import { escPend, slug, txtOn, haversine, featurePoint, currentSeason, wc } from './util.js';
...
import { isPicking, _pickSegs, initPicking } from './picking.js';
  ...
  // Render loop (render.js): the place hook it still needs from here.
  initRender({openLocalFeature});
  ...
  // runs after the whole body. isPicking/bumpPlaceReq are closures, not values:
  ...
  // Ride-check lives in ./ride-check.js. Coverage, item-index, the catalogue and
  // now the drawer are plain imports there; what is left injected is only what
  // places.js will own (2026-07-26-map-js-module-split-design.md §5). Closures
  // where the binding is live: _placeReq is a counter, not a value.
  initRideCheck({
    openRouteById,
