...
(function () {
  ...
  window.Cc.mountClimbEditor = function (opts) {
    ...
    // initial.route / initial.steep.at are stored [lat,lng]; reverse to [lng,lat].
    ...
    function undo() {
      ...
      // Invalidate in-flight work or a late resolve repaints the undone route.
      routeSeq++; profileSeq++;
      abortRoute();
      abortProfile();
      ...
      window.Cc.profileFromRoute(state.route, ctl.signal, manualAt).then(function (res) {
        ...
        if (!res) {
          ...
          /* Hand-placed marker stays; automatic one is re-derived when the line
             ...
             one only if the route no longer passes it. */
    ...
    function nearestGradPct(ll) {
      ...
      // Fraction by distance along the route, not vertex index (OSRM packs curves).
    ...
    function writeHidden() {
      ...
      // Derived, never typed (docs/specs/climb-elevation.md).
    ...
    function bindDrag(marker, handler) {
      // Snapshot on dragstart: by dragend the marker is already at its new position.
    ...
    function mkMarker(ll, cls, draggable, labelText) {
      // Rider steepest is amber/filled so it is not mistaken for the derived marker.
      ...
      // MapLibre sets pointer-events:none on the marker root; opt back in or drag never engages.
