...
final class RiderProfileTest extends WebTestCase
{
    ...
    public function testCountersCountApprovedWorkOnly(): void
    {
        ...
        $em->persist($consent);
        $live = new MediaUpload(Uuid::v4(), $uid, $consent->getId(), 'EU', 1200, 900, 4242, shard: 'EU-01', bucket: 'test-bucket-eu-01');
        $live->approve(null);
        $em->persist($live);
        $gone = new MediaUpload(Uuid::v4(), $uid, $consent->getId(), 'EU', 1200, 900, 4242, shard: 'EU-01', bucket: 'test-bucket-eu-01');
        $gone->approve(null);
        $gone->markObjectsDeleted();
        $em->persist($gone);
        $em->persist(new MediaUpload(Uuid::v4(), $uid, $consent->getId(), 'EU', 1200, 900, 4242, shard: 'EU-01', bucket: 'test-bucket-eu-01'));
        $em->flush();
