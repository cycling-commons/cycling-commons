...
final class MediaPersistenceTest extends KernelTestCase
{
    ...
    public function testUploadLifecycle(): void
    {
        ...
        $id = Uuid::v4();
        $upload = new MediaUpload($id, 7, $consent->getId(), 'EU', 3840, 2560, 123456, new \DateTimeImmutable('2025-10-04 09:12:33'), 50.4917, 5.8533, shard: 'EU-01', bucket: 'test-bucket-eu-01');
    ...
    public function testAnonymizeDropsTheAccountLinkAndFreezesTheCredit(): void
    {
        $consent = $this->consent();
        $id = Uuid::v4();
        $upload = new MediaUpload($id, 7, $consent->getId(), 'EU', 900, 600, 4242, shard: 'EU-01', bucket: 'test-bucket-eu-01');
        $upload->approve(99);
        $this->em->persist($upload);
        $this->em->flush();
    ...
    public function testEventsAppendAndOrder(): void
    {
        $consent = $this->consent();
        $id = Uuid::v4();
        $this->em->persist(new MediaUpload($id, 7, $consent->getId(), 'EU', 900, 600, 4242, shard: 'EU-01', bucket: 'test-bucket-eu-01'));
