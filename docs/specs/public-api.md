...
- **Licensing** (data ODbL/DbCL, media CC BY-SA, code AGPL-3.0-only):
...
hydrated alike, carry `© OpenStreetMap contributors` (§5).
...
English-only). The document is authored before the implementation, so endpoint-
...
## 3. Auth and metering: two mechanisms
