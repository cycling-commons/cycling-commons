...
| `pipeline` | build `pipeline/` (`python:3.12-slim`, non-root uid 1000) | `PIPELINE_PORT`: compose default **8002**, `.env.example` sets **8012** → 8000 | always | bind `pipeline/ → /app`; `DEM_DIR` (default `./data/dem`) → `/data/dem:ro`; healthcheck `/health` |
