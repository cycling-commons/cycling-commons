...
**Status:** canonical reference · **specified, pending implementation** · **Audience:** contributors to Cycling Commons
...
Until this contract is implemented, the only write path for translations is
editing `web/translations/messages.{en,fr,nl,de,es}.yaml` in git.
...
- Curator “revert this key to YAML” control (until then: delete the overlay
  row by a one-off / command).
