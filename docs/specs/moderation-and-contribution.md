...
Pinning is the author's, or an administrator's, from the post's edit page;
the board itself carries no pin control (owner 2026-09-19). A group trusted
to destroy a contribution is trusted to pin a note about it, but the person
who wrote the note decides how far it reaches. A direct
...
The board only lists (owner 2026-09-19): a **New post** button at the right
of the filter chips opens the composer on its own page, `/moderate/room/new`
...
and **Edited <date>** only once something changed, then a small pencil for
whoever may change it: the author, or an administrator. **Every change
happens on the edit page**: words, category, recipient, submission, pin,
pictures, and deleting the post, which sits below the form, says what it
deletes, and asks first. The board carries no control that changes or
destroys anything.
