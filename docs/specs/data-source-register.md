...
rather than copying it. A bulk mirror of a permissive source can still be
...
4.0 (CC BY-SA 4.0); wiki prose under CC BY-SA 4.0; code, and the interface
strings that ship with it, under **AGPL-3.0-only**
([`osm-data-architecture.md`](osm-data-architecture.md) §3). Everything
below follows from that.

The code licence does not enter this test. AGPL governs the software and says
nothing about the database; ODbL governs the database and says nothing about
the software. A source's terms are judged here only against **ODbL**.
...
- **No.** The terms forbid it. Recorded so it is not researched a third time.
...
letter already has OSM behind it, which is the baseline and is not repeated per
...
| Copernicus GLO-30 **D**igital **E**levation **M**odel (DEM) | The elevation behind every measured gradient | Free, attribution required | **Ingest**, in use, with the notice on `/credits` | verified (in use) |
...
| Esri World Imagery | Satellite base | ArcGIS Location Platform key, referrer-restricted, expires 2027-08-10 | **Reference**, in use under a key, with the terms recorded | verified (in use) |
...
exists, since the answer is the coverage plane), climbs (editorial by nature), and
