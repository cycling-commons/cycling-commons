...
| D7 | **Quote real code, and gate it against drift** | Originally "short excerpts, path citations" — cite `File.php` + symbol rather than paste, on the grounds that a citation rots slower. Revised 2026-07-23 on the owner's point: *"What we teach has to align with the code example. If code changes this could become incorrect."* Citations do not rot slower, they rot **silently** — a stale quote still builds, still reads fluently, and a reader cannot tell. So the wiki quotes real code, and `tools/check-wiki-code-drift.py` verifies every quoted line still exists in the file it claims to come from. Marking the existing 35 fences found 7 that had already drifted. A block that is not a quote is marked `CODE-ILLUSTRATIVE` with a note, so the reader always knows which they are looking at. The licence boundary still holds: excerpts stay short, never whole functions |
...
| U40 | 4 | absent | Reverse geocoding (coordinates → place name) | We only ever forward-geocode. `base_place` is stored as its own column at the moment a rider picks a town from search, specifically so the "Near Namur · 40 km" label never needs a live reverse lookup (`2026-07-19-region-scoping-design.md` §7) | open — owner review |
